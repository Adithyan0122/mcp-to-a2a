"""
Order Agent — v0.6 MCP + A2A
Runs on port 8001.
Receives restock requests via A2A, creates orders in pipeline_db,
then notifies the Notification Agent.
"""
...
def init_db():
    ...
    cur.close()
    conn.close()
    log.info("Orders table initialized in pipeline_db")
...
def notify(event_type: str, product: str, details: dict):
    try:
        httpx.post(
            ...
            json={"task": "send_alert", "event_type": event_type, "product": product, "details": details},
            timeout=5
        )
...
async def handle_a2a(request: Request):
    try:
        ...
        log.info(f"A2A request: task={task} product={product} quantity={quantity}")
        ...
        if task == "place_order":
            ...
            notify("order_placed", product, {"order_id": order["id"], "quantity": quantity})
