"""
Supplier Agent — v1.0
Single codebase for all 3 suppliers. Reads personality from environment.
Ports: SupplierA=8011, SupplierB=8012, SupplierC=8013
"""
...
AGENT_CARD = {
    "name": SUPPLIER_NAME, "version": "1.0.0",
...
app = FastAPI(title=f"Supplier — {SUPPLIER_NAME} v1.0")
...
async def handle_a2a(request: Request):
    ...
    try:
        ...
        return JSONResponse(content={
            "status": "accepted", "supplier": SUPPLIER_NAME,
...
async def health():
    return JSONResponse(content=build_health_response(
        SUPPLIER_NAME, "1.0.0", db_connected=True,
        extra={"bids_processed": bid_count, "port": PORT,
               "price_multiplier": BASE_PRICE_MULTIPLIER,
               "delivery_days": DELIVERY_DAYS, "reliability": RELIABILITY}
    ))

if __name__ == "__main__":
    log.info(f"{SUPPLIER_NAME} v1.0 starting on port {PORT} | "
             f"{BASE_PRICE_MULTIPLIER}x price | {DELIVERY_DAYS}d delivery | "
             f"{int(RELIABILITY*100)}% reliable")
